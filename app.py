    ...
    with tab1:
        ...
        if st.session_state.balance_start and st.session_state.balance_end:
            ...
            if balance_sec >= 30:
                ...
                st.warning("Below average. Consider mentioning this to a doctor if you've noticed recent changes.")
            db.save_health_checkin(st.session_state.health_session_id, balance_sec=balance_sec)
            trends = db.get_health_trends(st.session_state.health_session_id)
            if len(trends) > 1:
                df = pd.DataFrame(trends)
                df = df.dropna(subset=['balance_sec']).sort_values('timestamp')
                st.line_chart(df.set_index('timestamp')['balance_sec'], use_container_width=True)

    # ----- FLEXIBILITY TEST -----
    with tab2:
        st.markdown("**Sit-and-Reach Flexibility Test**")
        st.write("Sit with legs straight. Reach forward and measure distance (cm) beyond your toes (negative if you can't reach).")
        ...
        if st.button("Save Flexibility Score"):
            ...
            if flex_cm >= 10:
                ...
                st.info("Average. Stretch regularly to improve.")
            else:
                st.warning("Below average. Regular stretching helps prevent injury.")
            db.save_health_checkin(st.session_state.health_session_id, flexibility_cm=flex_cm)
            trends = db.get_health_trends(st.session_state.health_session_id)
            if len(trends) > 1:
                df = pd.DataFrame(trends)
                df = df.dropna(subset=['flexibility_cm']).sort_values('timestamp')
                st.line_chart(df.set_index('timestamp')['flexibility_cm'], use_container_width=True)

    # ----- REACTION TIME TEST -----
    with tab3:
        st.markdown("**Reaction Time Test**")
        st.write("Click the button as soon as it turns green.")
        ...
        if st.session_state.reaction_stage == "ready":
            ...
            if len(trends) > 1:
                df = pd.DataFrame(trends)
                df = df.dropna(subset=['reaction_ms']).sort_values('timestamp')
                st.line_chart(df.set_index('timestamp')['reaction_ms'], use_container_width=True)
